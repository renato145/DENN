...
for freq in [0.5,4]:	
	for diversity_method in ['CwN', 'Cwc', 'CwcN']:#'RI','Cw', 'None', 'Cwc', 'CwN', CwcN'
		for exp in ['exp1','exp2','exp3','exp4']:
			for func in ['sphere','rosenbrock','rastrigin']:
				# train_window = None if sample_size==1 else 5
				# epochs       = 10   if sample_size==1 else 3
				lines.append(f'''sbatch --export=ALL,experiment="{exp}",func_name="{func}",method="noNN",frequency="{freq}",frequency_save="{freq}",diversity_method={diversity_method},save="True",pbar="False",silent="False",cluster="True" cluster_job.sh\n''')
